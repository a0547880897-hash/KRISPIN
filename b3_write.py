"""Write batch2 visual-scan data (b3_scan.json) into the 18 new drafts (b3_map.json).
...
SCAN = json.load(open("b3_scan.json", encoding="utf-8"))
MAP = json.load(open("b3_map.json"))
...
def main():
    ...
    for nn in sorted(SCAN):
        ...
        time.sleep(0.35)
    if apply:
        json.dump(log, open("b3_write_log.json", "w"), ensure_ascii=False, indent=1)
