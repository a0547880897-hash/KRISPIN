"""Write batch2 visual-scan data (b2_scan.json) into the 18 new drafts (b2_map.json).
...
SCAN = json.load(open("b2_scan.json", encoding="utf-8"))
MAP = json.load(open("b2_map.json"))
...
def main():
    ...
    for nn in sorted(SCAN):
        ...
        time.sleep(0.35)
    if apply:
        json.dump(log, open("b2_write_log.json", "w"), ensure_ascii=False, indent=1)
