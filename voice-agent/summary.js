...
async function summarizeTranscript(transcript) {
  ...
  const prompt = `להלן תמליל שיחת מכירה בין תגל (נציגה דיגיטלית של בנק יהב) ללקוח. הפק דוח בעברית, ענייני וקצר, במבנה הבא בדיוק (אם פרט לא נמסר — כתוב "לא נמסר"):

שם הלקוח:
שכיר/עצמאי + פרטים שנמסרו (משכורת, הלוואות קיימות וכו'):
מה בחר: (הלוואה / פתיחת חשבון בלי עמלות / כלום)
פרטי הלוואה: (סכום שביקש, מספר תשלומים, סוג ההלוואה אם זוהה)
מספר מעקב שנמסר ללקוח (אם ניתן):
האם הועבר לתגל האנושית / צעד הבא:
